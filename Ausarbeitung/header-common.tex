...
%   Diese Datei enthält primär \usepackage-Aufrufe sowie eine kleine Menge
%   bewusst gewählter, stilunabhängiger Konfiguration, die sowohl mit dem
%   Beamer-Dokumentenstil (Vortrag/) als auch mit scrartcl (Ausarbeitung/) sicher
%   kompatibel ist (z.B. gemeinsame TikZ-/PGFPlots-Bibliotheken via
%   \usetikzlibrary oder \pgfplotsset).
%
%   Projekt- oder layoutspezifische Konfigurationsbefehle (\definecolor,
%   \hypersetup, \setlist, \newtheorem, umfangreiche \tikzset-Anweisungen,
%   \newcommand, \lstset usw.) dürfen NICHT hier stehen, weil Beamer viele davon
%   intern bereits setzt; doppelte Definitionen erzeugen Konflikte oder
%   überschreiben das Beamer-Theme stillschweigend.
...
\usepackage{iftex}
\ifPDFTeX
  % Bei pdfLaTeX sicherstellen, dass die Schriftkodierung vor `textcomp`
  % gesetzt wird, damit TS1-Symbole korrekt aufgelöst werden.
...
% Syntax-hervorgehobene Quelltextdarstellung. Nur das \usepackage steht hier;
% die Konfiguration (\lstset, \lstdefinelanguage{pseudocode}) steht in
% header-article.tex (Ausarbeitung-Kontext).
%
% Grund: \lstset verwendet \color{codeblue} für keywordstyle. Die Farbe
% codeblue wird erst in header-article.tex per \definecolor definiert.
% Würde \lstset hier stehen, käme es zu einem „Undefined color `codeblue'"
% -Fehler, weil header-common.tex vor header-article.tex eingelesen wird.
% (Analoges gilt für header-beamer.tex im Vortrag-Kontext.)
\usepackage{listings}
