...
**Benefits of latexmk**:
- Automatically runs LaTeX and Biber the correct number of times
...
# Process bibliography (this project uses biber backend)
biber Ausarbeitung
...
3. **Compile**: Both documents compiled with latexmk; biber backend used automatically
...
**Note on `Vortrag.bcf`:** This file is a BibLaTeX control file generated by biber as part of the bibliography processing. Both documents use `backend=biber` (see `Vortrag.tex` and `Ausarbeitung.tex`), so `Vortrag.bcf` is an expected build artifact committed alongside the other build files.
...
**Solution**: Install Biber (included with TeX Live)
...
**Solution**: Ensure Biber is configured correctly (this project uses `backend=biber`)
...
$bibtex_use = 2;  # Use biber for bibliography (matches backend=biber)
