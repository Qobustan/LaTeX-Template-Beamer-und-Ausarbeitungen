% ==============================================================================
% header-article.tex
% Article-spezifische Konfiguration (scrartcl / scrreprt / scrbook)
% Article-specific configuration
%
% Diese Datei enthält alles, was NICHT in header-common.tex stehen darf, weil
% es entweder mit Beamer inkompatibel ist oder nur für Ausarbeitungen sinnvoll:
%   - amsthm       → Beamer definiert eigene Theorem-Umgebungen; ein zweites
%                    amsthm-Load würde diese überschreiben bzw. zu Konflikten führen
%   - changepage / adjustbox  → Beamer-inkompatibel
%   - \definecolor / \hypersetup → Beamer hat ein eigenes Farbsystem; Definitionen
%                    müssen im Beamer-Theme gesetzt werden, nicht global
%   - \setlist     → Beamer setzt Listenabstände intern; globale Überschreibung
%                    würde Folien-Layout zerstören
%   - \newtheorem  → Wie amsthm (s.o.)
%   - \tikzset     → TikZ-Styles können mit Beamers Overlay-Mechanismus kollidieren
%   - \newcommand  → Mathematische Makros und Hilfsbefehle, nur für Ausarbeitungen
%   - \lstset / \lstdefinelanguage → Code-Listing-Konfiguration; Beamer nutzt
%                    üblicherweise eigene Blöcke (verbatim-Umgebungen)
%   - Unicode-Fixes (\newunicodechar) → Nur für pdfLaTeX-basierte Ausarbeitungen
%   - \subsubsubsection → Gliederungstiefe 4 nur für lange Ausarbeitungen relevant
%
% ==============================================================================
...
% Erweiterung: Theorem-Umgebungen (amsthm darf NICHT in header-common.tex stehen,
% da Beamer ein eigenes Theorem-System mitbringt und amsthm dort zu Konflikten führt)
\usepackage{amsthm}
...
% Listings-Konfiguration (VERSCHOBEN aus header-common.tex).
% Grund: \lstset{keywordstyle=\bfseries\color{codeblue}} referenziert die
% Farbe codeblue, die weiter oben in diesem Header per \definecolor definiert
% wird. Würde \lstset in header-common.tex stehen, wäre codeblue dort noch
% unbekannt → „Undefined color `codeblue'" beim Compilieren.
% (Gleiches gilt für header-beamer.tex im Vortrag-Kontext.)

% Pseudocode-Sprachdefinition für Listings
...
% Globale Listing-Einstellungen (VERSCHOBEN aus header-common.tex, s. Hinweis oben)
\lstset{%
	frame=single,
	framesep=0mm,
...
\subject{Schriftliche Ausarbeitung}
\def \vorlesung {Seminar: Name der Veranstaltung}
\def \semester {Semester Jahr}
\def \dozent {Prof. Dr. Name}
\date{Stand: \today}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
